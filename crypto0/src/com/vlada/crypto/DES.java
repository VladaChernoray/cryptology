package com.vlada.crypto;

import sun.security.util.BitArray;
import static com.vlada.crypto.Helper.*;
...
public class DES {
    ...
    public String encrypt(String input, String keyS, String initStr) {
        ...
        for (int i = 0; i < input.length() / 8; i++) {
            BitArray array = new BitArray(64, input.substring(i * 8, (i + 1) * 8).getBytes());

            array = xor(previousEncryptedBlock, array);

            array = replaseS(array);
            array = feistelEncrypt(array, key);
            array = replaseB(array);

            previousEncryptedBlock = array;
            output = output.concat(new String(array.toByteArray()));
        }
    ...
    public String decrypt(String input, String keyS, String initStr) {
        ...
        for (int i = 0; i < input.length() / 8; i++) {
            ...
            array=replaseS(array);
            array=feistelDecrypt(array, key);
            array=replaseB(array);

            array = xor(previousEncryptedBlock, array);
            result = result.concat(new String(array.toByteArray()));
        }
